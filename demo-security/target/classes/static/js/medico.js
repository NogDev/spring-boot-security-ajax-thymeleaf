...
$(function() {
	...
	// transforma string em um array retirando a virgula e o espaço em branco.
	function split(val) {
		return val.split(/,\s*/);
	}
	
	// extrai o ultimo elemento de uma array
	function extractLast(term) {
		return split(term).pop();
	}
	
	// adiciona a tag de input com a especialização do html
	function addEspecializacao(titulo) {
		$('#listaEspecializacoes')
			.append('<input type="hidden" value="'+ titulo +'" name="especialidades">');
	}
	
	// mostra na pagina um toast (Bootstrap) com mensagem de especialiadades repetidadas
	...
	$("#autocomplete-especialidades")
		...
		// keydown: ativa a função sempre que uma tecla é digitada
		...
		.autocomplete({
			...
			//quantidade minima de caracteres a ser digitados para realizar o request
			search : function() {
				// custom minLength
